...
def step(): # шаг
    for i in range(len(space_objects)): # текущий
        for j in range(len(space_objects)): # второй
            ...
            r = r ** 0.5 # R
            if abs(dx) > space_objects[i].r + space_objects[j].r or abs(dy) > space_objects[i].r + space_objects[j].r:
                ax = a * dx / r # a * cos
                ay = a * dy / r # a * sin
...
if __name__ == '__main__':
    ...
    space_objects = []
    aStar = obj(300, 300, star_img, 16, 1) # x, y, img, r, m
    space_objects.append(aStar)
    #aPlanet = obj(500, 500, planet_img, 35, 1000)
    #space_objects.append(aPlanet)
