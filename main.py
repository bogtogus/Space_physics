...
class obj:
    ...
    def update(self, zoom=1.0) -> None:
        if self.img_name == 'circle':
            return
        if self.img.get_width() * zoom > 3 and self.img.get_height() * \
                zoom > 3:  # if the size of the picture on the screen exceeds 3 px
            self.transformed_img = pygame.transform.scale(
                self.img, (int(self.img.get_width() * zoom), int(self.img.get_height() * zoom)))
        else:
    ...
    def draw(
        ...
        if position_on_display[0] - self.r * DIST_COEFF > user.size[0] or \
                position_on_display[0] + self.r * DIST_COEFF < 0 or \
                position_on_display[1] - self.r * DIST_COEFF > user.size[1] or \
                position_on_display[1] + self.r * DIST_COEFF < 0:
        ...
        if mode:
            ...
            x_coord = (self.x * DIST_COEFF +
                       movement[0] + self.r * DIST_COEFF) * zoom
            y_coord = (self.y * DIST_COEFF +
                       movement[1] - self.r * DIST_COEFF) * zoom
            if (self.vy != 0 or self.vx != 0):
                sin_ = self.vy / (hypot(self.vy, self.vx))
                cos_ = self.vx / (hypot(self.vy, self.vx))
                start = [(self.x * DIST_COEFF + movement[0]) * zoom,
                         (self.y * DIST_COEFF + movement[1]) * zoom]  # start point of line
                end = [(self.x * DIST_COEFF + movement[0] + 100 * cos_) * zoom,
                       (self.y * DIST_COEFF + movement[1] + 100 * sin_) * zoom]  # end point of line
                rotation = degrees(atan2(start[1] - end[1], end[0] - start[0])) + 90
                pygame.draw.line(display_surf, '#c80000', start, end, 2)  # direction line
                pygame.draw.polygon(display_surf,
                                    '#c80000',
...
def draw_text(
    ...
    Works slowly with multiple calls."""
...
def choosing_window(
    ...
    text_backgr = pygame.Rect(
        ...
        simulations_content[first_button][2].get_width() + 20 * COEFFICIENT, 5.25 *
        simulations_content[first_button][1].size[1])
...
def ingame_main_menu(
    ...
    while game_menu_cycle:
        ...
        if menu_buttons['continue_button'].body.collidepoint(
            ...
            name_of_saved = 'save-' + str(strftime(
                '%x %X', 
                strptime(ctime()))).replace('/', '-').replace(':', '-')
            #  search for similar folder
            for item in os.listdir(HOME_DIR + 'simulations/'):
                if not(item.startswith('.')) and not(
                    os.path.isfile(
                        os.path.join('simulations/', item))):
...
def simulation_loop(
    ...
    while sim_cycle:
        ...
        if moving_right:
            movement_obj[0] += movement_speed / zoom
        if not(pause):
            step(space_objects=space_objects, DT=DT, G=G)
            simulation_time += DT
...
if __name__ == '__main__':
    ...
    # transformation ratio of all GUI elements
    ...
    HOME_DIR = os.path.dirname(
        os.path.abspath(filename)).replace(
        "\\", "/") + "/"
